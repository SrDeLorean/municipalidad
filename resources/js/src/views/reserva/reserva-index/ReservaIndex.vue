<template>
  ...
  >
    ...
    >
      ...
      <!-- Column: Client -->
      <template #cell(get_usuario)="data">
        <b-media vertical-align="center">
          <template #aside>
            <b-avatar
              size="32"
              :src="data.item.get_usuario.avatar"
              :text="avatarText(data.item.get_usuario.fullname)"
              :variant="`light-${resolveClientAvatarVariant(data.item.invoiceStatus)}`"
            />
          </template>
          <span class="font-weight-bold d-block text-nowrap">
            {{ data.item.get_usuario.fullname }}
          </span>
          <small class="text-muted">{{ data.item.get_usuario.email }}</small>
        </b-media>
      </template>
      ...
      <!-- Column: Balance -->
      <template #cell(get_estado)="data">
        <template v-if="data.item.get_estado.id === 1">
          <b-badge
            pill
            variant="light-warning"
          >
            {{ data.item.get_estado.descripcion }}
          </b-badge>
        </template>
        <template v-if="data.item.get_estado.id === 2">
          <b-badge
            pill
        ...
        <template v-if="data.item.get_estado.id === 3">
          <b-badge
            pill
